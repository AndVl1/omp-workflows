...
### Notes
- Wave-001 bare command discoverability is intact: `/do-work`, `/team`, `/cto` remain owned by the external fullstack plugin (`registerWorkflowCommands`; exported surface unchanged), and command presentation copy is now derived from the actually registered names, so bare output stays byte-for-byte identical while `namespace='omp'` renders the `/omp-do-work`, `/omp-team`, `/omp-cto` equivalents; the internal bundle registers only `omp-*` names.
