...
- **`lecture_acquire` tool and provider-neutral acquisition boundary** — fullstack registers the main-session `lecture_acquire` tool, which fail-closes on non-main-session context, non-`lecture-research` workflows, or wrong-stage invocations, parses the bounded source set from `lecture_intake`, normalizes timestamped provider evidence, and persists `lecture_acquisition`. Core declares the `LectureAcquisitionPort`/contract boundary and never fetches URLs: provider credentials, rights, and setup are installation concerns owned by the consumer that registers the tool. If the tool/provider is unavailable or acquisition fails, the workflow stops — it never asks the user for a transcript; only `succeeded` or evidence-bearing `partial` acquisition advances while every unresolved source/provider failure is preserved.
- **Rights-gated acquisition/ASR/analysis pipeline** — intake records explicit rights and media mode; absent approval is metadata-only and fail-closed (`RIGHTS_REQUIRED`). Public-URL evidence keeps the legacy Gemini provider when configured, while owned audio runs the rights-gated `transcribe-analyze` pipeline: yt-dlp owned-audio download, bounded WAV validation and audio preprocessing, ASR via whisper-local, a hosted `whisper-1` endpoint, or OpenRouter native, then bounded text analysis — each stage rejecting malformed provider output instead of turning it into a fake transcript.
- **Native OpenRouter ASR transport** — posts to OpenRouter's `/api/v1/audio/transcriptions` with frame-aligned bounded chunking (chunk duration and per-chunk character caps, per-source request limits, bounded text splitting), conservative cost reservation against `maxProviderCostCents` before dispatch, request-level and per-chunk deadlines with abort propagation, and estimated timestamps when the provider returns plain text.
- **Bounded acquisition limits and retention** — hard limits on audio bytes and duration, request/response sizes, transcript characters and segments, attempts, and deadlines (`HARD_ACQUISITION_LIMITS`); ephemeral audio leases cap in-memory bytes and their cleanup is verified; provider payloads are classified without being retained, and normalized evidence is bounded per source.
- **Lecture eval harness** — `@andvl1/omp-workflows-fullstack/lecture-acquisition/eval` export plus `evals/lecture-eval-fixtures.json`: WER/CER, timestamp alignment, grounded-claim precision, proposal relevance, cost per minute, and cleanup verification aggregated into a bounded quality score for comparing acquisition providers and settings.

### Changed
- **Contracts and docs for the lecture workflow** — `lecture_intake`/`lecture_acquisition` artifact schemas in `artifacts-schema.json`, the LECTURE_RESEARCH row in the workflow resolution matrix, URL-first contract text in `/do-work`, `/cto`, and classification prompts, the `lecture-research` profile (`workflows/lecture-research.json`), and consumer documentation of the provider matrix and setup boundary in the fullstack README.
