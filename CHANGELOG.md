...
### Verified
- 271 unit tests green (core 98, fullstack 83, e2e 100 — was 253), typecheck + build clean. Live probe matrix in `/tmp/cto-dispatch-exp` (simple + heavy nested chains, MiniMax vs deepseek task role, direct vs nested dispatch); context-hook delivery probes in `/tmp/hook-probe`; standby/inbox/wake live flow in `/tmp/cto-standby-live`.
