...
## [Unreleased]
### Added
- **Typed cross-profile handoff route catalogue** — the engine now ships a typed, default-deny transition catalogue (`HANDOFF_ROUTE_CATALOGUE`, live snapshot via `handoffRouteCatalogue()`) with stable route ids, semantics kinds, explicit dispositions (`enabled` | `conditional` | `unsupported`), source/target stages, prerequisites and UX metadata. `spec-preparation:handoff` → `full-feature:discovery` stays enabled; post-feature regression, regression→bug-fix/debug-cycle, and post-fix feedback routes are catalogued as `conditional` and rejected deterministically (rejection carries the route metadata and the missing `blocked_by` evidence/materialization adapters) until their adapters exist; documented unsupported direct pairs (spec→bug-fix/debug-cycle, feature→bug-fix/debug-cycle, regression→feature, review/analysis→implementation, same-profile) and unknown target strings fail closed. Route ids/keys are unique by construction, `workflow_handoff` remains a distinct main-session control tool beside `workflow_advance`, the safe result exposes route id/kind/status, preparation and bounded carried references, `/do-work` selects targets from the catalogue, and the matrix is documented in `packages/core/workflows/README.md`.
- **Cross-profile workflow handoff** — an approved, completed run now transfers atomically into a registered target workflow through the main-session `workflow_handoff` control tool, backed by an engine-owned route registry (first route: `spec-preparation` final stage `handoff` → `full-feature` entry stage `discovery`; generic `registerWorkflowHandoffRoute` for future routes). The transition validates the source capability/binding, source profile hash, terminal source shape and produced artifacts, the registered route, target profile/hash/stage, typed approval evidence (a durable `approved` checkpoint decision or a bound `workflow_approval` artifact — free text never authorizes), and bounded context, then performs exactly one atomic state write that preserves task/history/artifacts/scope/checkpoint decisions and bounded context, sets `workflow_override: true` with the target profile hash and entry stage `in_progress`, rotates capability identity/secrets (only hashes are persisted, never plaintext tokens), and appends a durable `HandoffRecord` audit entry. Unknown profiles/routes, stale credentials, profile-hash drift, incomplete sources, missing/invalid approval, and unsafe or oversized context fail closed with byte-identical canonical state. Engine: `packages/core/src/engine/durable.ts` (`handoffWorkflow`, `registerWorkflowHandoffRoute`) and `types.ts`; tool registration: `packages/fullstack/src/index.ts`; prompt protocol: `packages/core/src/commands/do-work.ts`; tests: `packages/core/test/handoff.test.ts`, `packages/core/test/do-work-autonomy.test.ts`, `packages/fullstack/test/workflow-tools.test.ts` (core 474/474 and fullstack 242/242 tests pass; both typechecks clean).
## [0.23.2] — 2026-08-24
### Fixed
