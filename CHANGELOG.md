...
## [0.27.0] — 2026-09-01
### Changed
- **Readable specification workflow** — replaces the JSON-only `spec-preparation` experience with one branch-independent `specs/<feature-id>/` workspace. Current phase, validation, approvals, constitution binding, next action, handoff, and the final implementation-conformance matrix are readable Markdown projections backed by durable typed state and immutable artifacts.
- **One shared workflow path** — direct `/specify`, `/spec-plan`, and `/spec-tasks` commands, read-only `/spec-import`, full specification preparation nested from `/do-work`, and CTO preparation/execution consume the same workspace, handoff, constitution prerequisite, checkpoint contract, and terminal conformance gate. Language and template selection is explicit and version-bound; CTO records each feature phase decision separately, and execution requires explicit current-user confirmation of the task-to-team/slice mapping. External framework adapters contribute bounded metadata only and are never required to generate or validate a workspace.
- **Durable migration and resume** — persisted legacy specification state is migrated deterministically with provenance, preserving valid upstream artifacts and the exact originating operation. A migrated, revised, stale, interrupted, or constitution-affected run resumes from its persisted cursor rather than recreating work; any required migration or changed semantic binding performs fresh validation and requires a new trusted human approval before the originating flow continues.
- **Adaptive execution and CTO fan-in** — bounded semantic roster selection preserves role multiplicity and freezes the selected snapshot before dispatch. `/do-work` and CTO execution acquire claims before dispatch, map implementation slices to exact handoff digests, and evaluate each feature independently through the same requirement-closure matrix; a generic task/DoD result or human acknowledgement cannot substitute for required implementation, review, test, or quality-gate evidence.

### Removed
- **JSON-only and legacy completion paths** — the old JSON-only specification flow and the generic `dod_and_artifacts` completion shape are retired from active commands, profiles, prompts, schemas, renderers, producers, and consumers. The clean cutover renames `CompletionDodStatus` to `CompletionQualityGateStatus` and `CompletionArtifactRef.dod_status` to `quality_gate_status`; no active alias, compatibility shim, second completion contract, or feature-specific Definition of Done remains.

### Migration
- **Consumer action required (breaking change)** — update imports, typed producers/consumers, serialized persisted records, profile/artifact schemas, prompts, and tests to use `quality_gates_and_artifacts`, `CompletionQualityGateStatus`, and `quality_gate_status`. Move integrations to the readable `specs/<feature-id>/` workspace and invoke `/spec-import` for supported existing local documents instead of the retired JSON-only path; use explicit `--feature` selection when a workspace is not uniquely resolvable. Do not add or depend on a compatibility alias.
- Existing persisted records are accepted only by the one-time migration reader, which records provenance and then requires current validation plus fresh human approval. After migration, revisions, changed intent, stale evidence, or changed constitution/template/language bindings must return to the affected specification phase; consumers must not bypass that checkpoint or reuse old approval.

### Security
- **Hard-stop invariants** — ambiguous workspace/provider/source selection, unauthorized or unsafe paths, malformed or missing constitution, unresolved template content, invalid ownership or claims, stale/failed/contradictory evidence, changed handoff digest, missing trusted approval, or incomplete requirement closure block dispatch or terminal completion without mutating canonical progress. Native generation and external compatibility validation cannot run before the shared constitution prerequisite succeeds; checkpoint approval cannot be inferred from policy, validation, an agent, CTO, or external status. Source instructions are never executed, external adapters cannot author or approve content, and a nested CTO cannot be created.

## [0.26.0] — 2026-08-29
### Changed
- **Adaptive semantic roster selection** — roster stages now expose their allowed role pool through `workflow_instructions` before any capability is issued, and `workflow_begin` accepts only semantic role/facet/focus/reason selections; concrete agent ids are rejected at both the tool boundary (strict zod schema) and the durable boundary. Selections are validated against the live registered agent mapping and fail closed on unmapped roles, multiplicity/bounds violations, or a missing trusted mapping; an identical re-issue against an active capability is idempotent, a changed selection is rejected naming the frozen snapshot. Advancing into a roster-policy stage defers selection and capability issuance until an explicit `workflow_begin`; non-roster stages arm normally at advance, and a loop `back_to` a roster stage defers the same way. Full-feature exploration and architecture stages moved from fixed alias manifests (`architect_minimal`/`architect_clean`/`architect_pragmatic`) to bounded 1..3 worker pools — analyst and tech-researcher each up to 3 while overall `max_workers` stays 3, so explicit analyst×3 or tech-researcher×2 selections are valid and normalize to stable numbered repeated-role slots — with deterministic defaults (minimum valid set plus at most one risk-trigger addition), and the alias presets were removed from shipped roles, fallback chains, examples and init-team output.
- **Private internal bundle activation via project extension setting** — `@andvl1/omp-workflows-internal` now activates through the supported OMP 18.x project extension setting (`<cwd>/.omp/settings.json#extensions`) and eagerly publishes namespaced `/omp-do-work`, `/omp-team`, `/omp-cto` commands plus the read-only `/omp-workflow-team validate` diagnostic; the `.omp/settings.json`, `.omp/team.config.json` and `.omp/teams.json` policy files are tracked through narrow `.gitignore` negation exceptions while all other `.omp` runtime content stays ignored. Ownership stays marker-gated: commands and the three engine capabilities are claimed only inside workspaces carrying the bundle marker (typed fail-closed `activation_markers_missing` elsewhere), all under a single private owner and idempotently across session restarts. Host agent discovery is bundle-owned and lazily injected: `session_start` kicks off the marker-gated mapping refresh, `workflow_begin` awaits it before issuing capabilities, discovery is filtered to exact `omp-*` candidates with no generic `task`/unrelated fallback, and the refreshed mapping feeds the omp-* roster while preserving custom external plugin agents and project-local role mappings; README documents the contract.
### Fixed
- **Canonical `teams[].dod_path` normalization** — one resolver (`resolveDodPath`/`readDoDFile` in `packages/core/src/engine/dod.ts`) now interprets `teams[].dod_path` for every consumer (slice gate, integration gate, session report, visualization): either the directory containing `dod.json` or the file itself, with unset paths defaulting to `.work-state/artifacts/<teamId>`. Traversal, absolute, NUL, oversized and symlinked paths fail closed without echoing the unsafe value; missing/malformed DoD diagnostics name the resolved file path plus cause instead of an empty pending list, and `/cto` prompts state both accepted forms explicitly.
### Notes
- Wave-001 bare command discoverability is intact: `/do-work`, `/team`, `/cto` are registered only by the external fullstack command surface (`registerWorkflowCommands`; exported surface unchanged), while command presentation copy derives from the actually registered names so bare output stays byte-for-byte identical and `namespace='omp'` renders `/omp-do-work`, `/omp-team`, `/omp-cto`. Command-map registration is distinct from workflow capability ownership: in this marked monorepo the internal bundle owns `workflow_registration`, `workflow_tools`, and `config_writer`, so a visible external owner-aware bare handler is not an internal fallback and fails closed on `owner_conflict`. The consumer docs now specify the supported choices: edit roster config, use `commandPrefix` for coexistence, or disable the old bundle and wire all three core seams for a complete replacement; copied `.omp/commands` files are not an override API.
- Archived workflow-v2 disposition unchanged: the blocked experiment remains only on `backup/workflow-v2-blocked-2026-08-29`; this branch carries wave-004 fixes and revives nothing from workflow-v2.
- Validation (Main-observed, after the final review rework): build PASS across all four workspaces; all-workspace typecheck PASS (fullstack source + custom commands); focused core suites 214/214; full core 746/746; e2e package 67 passed / 0 failed / 7 skipped (node-pty fixtures unavailable); fullstack 277/277; internal 74/74; `git diff --check` clean. Live OMP 18.0.6 smoke: the marked-workspace command inventory lists `omp-workflow-team`, `omp-do-work`, `omp-team`, `omp-cto` (bare `do-work`/`team`/`cto` appear only from the separately active fullstack plugin); `/omp-workflow-team validate` reports markers OK with `workflow_registration`, `workflow_tools` and `config_writer` all under the single `private_omp` owner and a 14-agent live mapping; an `/omp-do-work` dispatch emitted the internal classification prompt with the eight-step instructions-before-begin contract; the unmarked-workspace negative smoke stays fail-closed (`activation_markers_missing`, zero owner claims). Fresh security and correctness re-reviews both ACCEPT the delivery after prior P1/P2 blockers were remediated and rechecked (contract epoch binding, internal activation ordering, visualization excluded-input planning); the one open item is the pre-existing, out-of-scope visualization read TOCTOU deferred to bead `br-w004-vis-toctou-read-w0l`. No version bump, tag, release, or push is implied.
...
- **Additive visualization API on `@andvl1/omp-workflows-core`** — the projection model, renderers, Markdown/HTML serializers, manifest/link preflight and the atomic publisher are exported additively through the root barrel. The `@andvl1/omp-workflows-core/visualize` subpath export exposes the barrel's own `SessionKind`/`ArtifactStatus`/`WorkflowName` types, which the package root intentionally resolves to the pre-existing report/engine types of those names. No breaking export changes, zero new runtime dependencies, Node >= 20.
- **Automatic command discovery** — `/workflow-view` ships through the existing dynamic copy-commands scan (`postinstall`, CLI, and `session_start` sync); no copy-command allow-list change was required.
