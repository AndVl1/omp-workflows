...
import { extractPayloadBetweenMarkers } from "./before-agent-start-marker.js";
import {
	type InventoryModel,
	type ModelRoleEntry,
	resolveRoleChain,
...
async function collectValidation(api: CustomCommandAPI, ctx: HookCommandContext, cwd: string): Promise<{ report: string; data: ValidationData; webSearchEnabled: boolean | null }> {
	...
	// HookCommandContext does not expose authStorage/ToolSession, so the toggle
	// is the only signal observable from a custom command.
...
function wrapResearchRequest(payload: string): string {
	// The legacy marker is an internal serialization step; registration replaces it
	// with a session-bound authorization envelope before it reaches the host.
...
const factory = (api: CustomCommandAPI): CustomCommand => ({
	...
	async execute(args: string[], ctx: HookCommandContext): Promise<string> {
		...
		// The extension-created API cwd is authoritative; never let a stale or
		// attacker-controlled command context redirect settings/file reads.
		const cwd = api.cwd;
		if (!cwd) {
...
export interface ModelRolesCommandRegistrationOptions {
	resolveCwd: (ctx: unknown) => string | undefined;
	/** Optional marker/root liveness gate; null rejects the command silently. */
	ensureLiveActivation?: (ctx: unknown, cwd: string) => string | null;
	/** Issue an ExtensionAPI/session-bound one-time research authorization. */
	issueResearchAuthorization?: (ctx: HookCommandContext, cwd: string, payload: string) => string | null;
}

function extensionCommandApi(pi: ExtensionAPI, cwd: string): CustomCommandAPI {
	return {
		cwd,
		exec: async () => ({ stdout: "", stderr: "", code: 0, killed: false }),
		typebox: pi.typebox,
		arktype: pi.arktype as CustomCommandAPI["arktype"],
		zod: pi.zod,
		pi: pi.pi,
	};
}

/** Register the shared command implementation through OMP's canonical API. */
export function registerModelRolesCommand(
	pi: ExtensionAPI,
	options: ModelRolesCommandRegistrationOptions,
): void {
	pi.registerCommand("omp-model-roles", {
		description: "Validate per-agent model roles or delegate fresh model recommendations.",
		handler: async (args, ctx) => {
			const tokens = args.trim() ? args.trim().split(/\s+/) : [];
			const requestedCwd = options.resolveCwd(ctx);
			if (!requestedCwd) return;
			const cwd = options.ensureLiveActivation
				? options.ensureLiveActivation(ctx, requestedCwd)
				: requestedCwd;
			if (!cwd) return;
			const command = factory(extensionCommandApi(pi, cwd));
			const result = await command.execute(tokens, ctx as unknown as HookCommandContext);
			if (options.ensureLiveActivation && options.ensureLiveActivation(ctx, requestedCwd) !== cwd) return;
			if (typeof result === "string") {
				if (options.issueResearchAuthorization && tokens.length === 1 && tokens[0] === "recommendations") {
					const payload = extractPayloadBetweenMarkers(result);
					if (!payload) return;
					const authorized = options.issueResearchAuthorization(ctx as unknown as HookCommandContext, cwd, payload);
					if (!authorized) return;
					pi.sendUserMessage(authorized);
					return;
				}
