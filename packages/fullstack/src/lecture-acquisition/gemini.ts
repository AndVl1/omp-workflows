...
function canonicalVideoUrl(url: string): string {
  let parsed: URL;
  try { parsed = new URL(url); } catch { throw new AcquisitionProviderError("INVALID_URL", "video URL is invalid", { provider: PROVIDER, retryable: false }); }
  ...
  if (hasForbiddenUrlParts || (!validWatchQuery && !validShortQuery)) throw new AcquisitionProviderError("UNSUPPORTED_URL", "canonical public YouTube video URL is required", { provider: PROVIDER, retryable: false });
  const id = isShort ? parsed.pathname.slice(1) : parsed.searchParams.get("v");
  if (!id || !/^[A-Za-z0-9_-]{11}$/.test(id)) throw new AcquisitionProviderError("INVALID_URL", "canonical public YouTube video URL is required", { provider: PROVIDER, retryable: false });
...
export class GeminiYouTubeProvider {
  ...
  constructor(private readonly options: GeminiYouTubeOptions) {
    try { this.endpoint = validateEndpoint(options.endpoint ?? DEFAULT_ENDPOINT, { trust: "official-google", provider: PROVIDER, officialHost: "gemini" }); }
    catch { throw new AcquisitionProviderError("INVALID_URL", "Gemini endpoint is not allowed", { provider: PROVIDER, retryable: false }); }
  }
  ...
  async analyzeYouTubeVideo(source: ResolvedVideoSource, request: LectureAcquisitionRequest, signal: AbortSignal): Promise<unknown> {
    ...
    try {
      const endpoint = endpointWithPath(this.endpoint, "/v1beta/interactions");
      const response = await this.options.fetch(endpoint, { method: "POST", headers: { "content-type": "application/json", "x-goog-api-key": this.options.apiKey }, body: JSON.stringify(payload), signal, redirect: "error" });
      const body = await readBoundedResponseText(response, this.options.maxResponseBytes ?? request.limits.maxResponseBytes, PROVIDER);
      if (!response.ok) throw classifyProviderHttpStatus(PROVIDER, response.status, "");
      let data: unknown;
      try { data = JSON.parse(body); } catch { throw new AcquisitionProviderError("INVALID_PROVIDER_RESPONSE", "provider returned invalid analysis data", { provider: PROVIDER, retryable: false }); }
      const text = extractText(data);
      if (!text) throw new AcquisitionProviderError("INVALID_PROVIDER_RESPONSE", "provider returned no analysis text", { provider: PROVIDER, retryable: false });
  ...
  async acquire(source: ResolvedVideoSource, request: LectureAcquisitionRequest, signal: AbortSignal): Promise<{ provider: string; raw: unknown }> {
    return { provider: PROVIDER, raw: await this.analyzeYouTubeVideo(source, request, signal) };
  }
}
...
function extractText(value: unknown): string | undefined {
  ...
  for (const group of [root.steps, root.outputs]) if (Array.isArray(group)) for (const item of group) {
    ...
    if (Array.isArray(content)) for (const part of content) if (part && typeof part === "object" && !Array.isArray(part) && "text" in part && typeof part.text === "string") return part.text;
    if ("text" in item && typeof item.text === "string") return item.text;
  }
  return undefined;
}

export async function analyzeYouTubeVideo(options: GeminiYouTubeOptions, source: ResolvedVideoSource, request: LectureAcquisitionRequest, signal: AbortSignal): Promise<unknown> {
  return new GeminiYouTubeProvider(options).analyzeYouTubeVideo(source, request, signal);
}
