...
const RESUME_FROM_DISK_STEPS = [
  ...
  "**Checkpoint/gate/advance** — checkpoint permission exists only when the returned stage contract declares it. Before `workflow_advance`, resolve human authorization with `workflow_checkpoint_ask`: it verifies the pending checkpoint, asks the human at the live terminal/RPC surface, and commits the answer through the engine's durable checkpoint ledger (headless sessions fail closed). Then call `workflow_checkpoint` with the same handoff identity (including `loop_iteration`) plus `checkpoint_id`, `checkpoint_kind`, `authorization`, `actor_provenance`, `decision`, and `rationale`, copying the ask result's decision, proof, and `loop_iteration` binding verbatim — a reconstructed proof or binding never authorizes, and legacy `mode`/`actor` fields cannot. Advance only after gates/evidence pass; after `workflow_advance`, call `workflow_instructions` again and remain in this session for later feedback.",
] as const;
