...
const REWORK_READ_ONLY_STEPS = [
  ...
  "Inspect `workflow_instructions.profile.stages` as ordered informational metadata (`id`, `title`, `type`, plus present `description`, `prompt`, `consumes`, `optional_consumes`, and `produces`). Map feedback semantically across the complete registered profile to the earliest actually affected stage and honor declared artifact dependencies.",
