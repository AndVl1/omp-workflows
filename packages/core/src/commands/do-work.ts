...
const REWORK_READ_ONLY_STEPS = [
  ...
  "Inspect `workflow_instructions.profile.stages` as ordered informational metadata (`id`, `title`, `type`, plus present `description`, `prompt`, `consumes`, `optional_consumes`, and `produces`). Before choosing `affected_stage`, compare the feedback and desired outcome with the saved task constraints and every relevant saved producer output (decisions, requirements, acceptance/DoD, and supporting evidence). Use the already-returned `required_input_contents`, declared canonical artifact paths, and ordered `profile.stages` `produces`/`consumes`; read missing relevant saved outputs at those declared paths read-only and never invent their contents.",
  "Distinguish a defective implementation of unchanged requirements from a changed requirement, decision, or acceptance. Attribute each incompatible output to its producer in the selected profile; preserve upstream outputs only when compatible with the revised requirement, then choose the earliest genuinely affected producer and downstream dependency work. A prior `met` state or current cursor is not proof of compatibility. Map feedback semantically across the complete registered profile.",
  "Do not use keyword, numeric-position, current-cursor, `implementation`, first-stage, or other heuristics/defaults. If substantive desired-result ambiguity remains, ask what outcome/result the user wants corrected and resolve it before mutation; NEVER ask the user to provide a stage ID.",
  "Only after the read-only target and mapping are verified, make the first mutation with `workflow_prepare`, supplying the exact selector/list snapshot/run id, unchanged command intent, feedback, and a non-blank `affected_stage`. If status/instructions discovery or semantic mapping is unavailable, return its structured error and stop.",
] as const;
...
export function buildDoWorkPrompt(envelope: ParsedWorkEnvelope, cwd: string): string {
  ...
  if (envelope.run_id) {
    ...
    continuation = [
      ...
      "Preserve the selected run's classification, artifacts, stage history, and prior task text. For rework, map feedback to the earliest affected stage from ordered profile.stages metadata and pass that non-blank affected_stage to workflow_prepare.",
  ...
  const lifecycleSelection = continuationMode
    ? [
      ...
      envelope.mode === "resume"
        ...
        : "For rework with or without a selector, complete the mandatory read-only workflow_status(selector) → workflow_instructions(same selector) discovery and semantic affected_stage mapping before the first `{ mode: \"rework\" }` workflow_prepare mutation; classification is optional at this lifecycle boundary.",
