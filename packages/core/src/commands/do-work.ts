...
export function buildDoWorkPrompt(envelope: ParsedWorkEnvelope, cwd: string): string {
  ...
  return [
    ...
    "After every delegated call or parallel batch, apply terminal-aware ordering: for a succeeded terminal row, validate the current stage schema, required artifacts, and advertised readiness requirements, then call `workflow_complete` FIRST with success and the exact newest handoff binding; for a failed/cancelled/non-success terminal row, call `workflow_complete` FIRST with the actual outcome/evidence and exact newest binding, without any success artifact/readiness claim; call `workflow_status`/reconciliation only after the accepted explicit completion. If no terminal row/result exists, and only then, use `workflow_status`, `background_wait`, or hub wait. Every delegated task payload must state that `workflow_*` control tools are main-session-only, must not mutate canonical `.work-state` with `bash`, and must use `write` for its declared artifact before returning. On the succeeded path, require every current-stage schema-declared artifact/field, including gate or validation evidence only when advertised, then dispatch the next stage only if the state transition is valid. A subagent return is not permission to improvise, skip stages, or self-complete.",
    ...
    "After every delegated call or parallel batch, use terminal-aware ordering: succeeded terminal row → current-stage schema/artifact/readiness validation → `workflow_complete` with success and exact newest binding → status after accepted join; failed/cancelled/non-success terminal row → `workflow_complete` FIRST with actual outcome/evidence and exact binding, without success artifact/readiness claim → status after accepted join; absent terminal row → `workflow_status`/`background_wait`/hub wait. A native task result is not artifact completion. Complete only the exact declared artifact IDs, including each consilium `slot_artifacts` ID, and advance only after current-stage dispatches, typed artifacts, and gates are complete. Never call `task` from a stale cursor.",
