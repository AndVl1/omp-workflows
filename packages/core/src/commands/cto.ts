...
export function buildCtoPrompt(envelope: ParsedCtoEnvelope, cwd: string, opts: CtoPromptOptions = {}): string {
  ...
  return [
    ...
    "A slice classified `LECTURE_RESEARCH` (transcript/playlist research) resolves deterministically to the",
    "`lecture-research` profile — a RESEARCH-ONLY workflow with an explicit human approval/stop gate. It is",
    "DISTINCT from generic `INVESTIGATION` -> `research`: it exists to turn transcripts/playlists into",
    "verifiable, actionable findings, never into code. Requirements:",
    ...
    "2. **Transcript-first intake with provenance**: findings MUST be grounded in the source",
    "   transcripts/playlists — every claim carries exact provenance (source id, timecode, quoted",
    "   evidence). No ungrounded synthesis.",
    "3. **Parallel bounded lecture mapping**: lectures are mapped by bounded parallel workers, then",
    "   synthesized and deduplicated (overlapping claims merged, conflicts recorded with the winning source).",
    ...
    "   `decision` escalation with `timeoutMs` + `default`). No implementation starts before approval; a",
...
export function buildAmendPrompt(
  ...
  return [
    ...
    "### LECTURE_RESEARCH slices (research-only, human-gated)",
    "A slice classified `LECTURE_RESEARCH` (transcript/playlist research) resolves deterministically to the",
    "`lecture-research` profile — a RESEARCH-ONLY workflow with an explicit human approval/stop gate, DISTINCT",
    "from generic `INVESTIGATION` -> `research`. Requirements for such teams:",
    ...
    "2. **Provenance/timecoded evidence**: every finding grounded in source transcripts/playlists (source id,",
    "   timecode, quoted evidence); parallel bounded lecture mapping, then synthesis with dedupe.",
    "3. **Repo-fit plus security review, READ-ONLY**: findings checked against the repository and security-",
    "   reviewed before being presented as actionable; no fixes.",
    "4. **Human approval/stop gate**: the wave ends at an explicit human approval checkpoint (`ask` or a",
    "   `decision` escalation with `timeoutMs` + `default`). NO implementation before approval; on rejection",
