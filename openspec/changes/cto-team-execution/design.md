...
- Получать проверяемые результаты небольшими связными пакетами, не накапливая зависимую непроверенную реализацию.
- Замыкать review/fix в команде и принимать решение о смене подхода при отсутствии сходимости.
...
- Событийная парковка координатора без модельных ходов и объединение progress-уведомлений относятся к отдельному будущему host/runtime change. Этот design не обещает устранить polling одними промптами и не ждёт реализации того change.
...
### 3.1. Проверяемый пакет внутри slice

Checklist implementation tasks не является форматом одного dispatch. До назначения реализации lead выделяет минимальный связный результат с packet identity, scope записи, требуемыми версиями входных контрактов, критерием результата и способом проверки. Пакет связан с child/revision и использует существующие стадии и циклы; отдельный interpreter, top-level claim или обязательный новый агент на каждый пакет не нужны.

Child contract/state сохраняет packet references и различает `implemented-unverified`, `locally-verified` и `integration-pending` как факты о результате, не как замену cursor профиля. Evidence указывает проверенные inputs/revision. Проверка выполняется после завершения конфликтующих writers, на стабильной версии; изменение inputs делает затронутое evidence неактуальным. Fixtures и подготовленный, но не запущенный сценарий не дают verification receipt.

Admission зависимой реализации проверяет receipt именно требуемого контракта. Непроверенный producer не открывает зависимую wave; независимая подготовка и непересекающаяся работа допустимы. Lead не обязан заранее дробить весь план, но обязан определить границу следующего пакета до dispatch. Его успешная проверка не означает завершения slice: полный cutover, профиль и итоговая интеграция сохраняются.

### 3.2. Внутренний review/fix и условия завершения

Reviewer возвращает находки lead, тот назначает исправления разрешённым writers и повторную проверку. CTO не является relay для штатных замечаний. Вверх идут только результат или исключение по матрице полномочий; диагностические материалы остаются доступны по ссылкам.

В existing child artifacts сохраняется реестр находок: стабильная identity, нарушенный критерий/инвариант, evidence, владелец, blocking disposition, условие закрытия и evidence закрытия. Повтор обновляет исходную запись. Приёмка проверяет отсутствие открытых обязательных находок и актуальность evidence. Новые нарушения исходного контракта, регрессии и обязательные security-гарантии блокируют результат; независимые улучшения вне scope требуют отдельного решения, а не автоматически расширяют пакет.

После fix повторно проверяются закрываемые находки и dependency impact set. Общий контракт, модель угроз или обязательный профиль могут потребовать полного повторного аудита; основание фиксируется. Нет ни автоматического полного аудита после каждого локального изменения, ни автоматической приёмки после заданного числа раундов.
...
Оценка сходимости сопоставляет закрытые критерии/находки, актуальное evidence, непроверенную работу и повторяющиеся классы ошибок. Число сообщений и объём кода не заменяют результат. Последовательное открытие новых достижимых дефектов после подтверждённых исправлений само по себе не является тупиком. Пороговые ограничения берутся из policy конкретного исполнения, не из универсальной константы.

Зафиксированный тупик блокирует расширение зависимой непроверенной работы, не независимые пакеты и не безопасное завершение текущих назначений. Решение CTO содержит выбранное действие (уменьшение пакета, пересмотр предпосылки, замена исполнителя, стабилизация основания), следующий проверяемый результат и применимые лимиты. Admission следующей попытки требует это решение вместе с budget authorization: увеличение allocation само по себе не снимает блокировку. Замена writer внутри обычного локального цикла остаётся полномочием lead; эскалация не нужна на каждое назначение.
...
| Status/report consumers | Readiness, blocked dependencies, delivery/application, текущая revision, packet readiness, обязательные findings, evidence и решение о сходимости |
...
- Пакеты: реализованный, но непроверенный producer не открывает зависимую реализацию; независимая подготовка продолжается; проверка стабильных inputs открывает нужную wave, изменение inputs требует актуализации evidence; fixtures без исполнения не дают успех.
- Review: реальная находка исправляется и закрывается внутри команды без relay CTO; повтор не создаёт новый блокер; локальный fix сохраняет независимое evidence, изменение общего контракта требует расширенной проверки, обязательный дефект не списывается как улучшение вне scope.
- Сходимость: непроверенный фронт не расширяется после зафиксированного тупика; одно продление бюджета не разрешает новый dispatch; решение с новым подходом и проверяемым результатом позволяет попытку в пределах полномочий.
