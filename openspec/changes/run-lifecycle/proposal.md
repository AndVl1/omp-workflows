...
- Переходы выполнять через `workflow_prepare` с явным mode и видимым результатом: прежний/выбранный запуск, статус и точка продолжения. Отдельная операция архивирования ordinary run не вводится.
- Сделать название задачи и выбор из понятного списка основным UX: UUID остаётся внутренним selector, пользователь не обязан его копировать.
- Resume в новой host-сессии без прежнего чата восстанавливает не только cursor, но и содержательный контекст из canonical state и обязательных артефактов этапов до дальнейшей реализации.
...
- Сохранить историю, артефакты и их provenance; перевести текстовые list/status, reports и observability на однозначную идентичность запуска. Визуализацию адаптировать только локальным подключением reader; если требуется переработка UI или модели графа, скрыть точки входа для нового формата и отложить переработку в отдельный change.
...
- Отдельный archive lifecycle и существенная переработка визуализации также вне scope. Отключённый viewer не блокирует lifecycle cutover; доступный viewer не имеет права читать прежнюю authority или показывать чужой запуск.
